...
- By default, training has two phases: cache the selected residual-stream activations as float32, then train and evaluate the SAE without the LLM loaded. `--streaming-mode` instead captures activations as they are used and keeps the LLM loaded without writing a residual cache.
