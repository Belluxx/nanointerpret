...
- **Input scaling:** One dataset-level scalar is estimated from a sample of training activations so that their average squared L2 norm equals the residual stream dimension. The same scalar is used for training, validation, and reporting. Activations are not normalized independently per token.
