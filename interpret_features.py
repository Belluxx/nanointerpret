...
def render_example(
    ...
    text = decode_marked_prefix(
        tokenizer, token_ids[context_start : token_position + 1]
    )
...
def choose_examples(
    ...
    for lower, upper, label in (
        ...
        for rank in rng.sample(range(start, stop), 2):
            examples.append(make_example(rank, label))

    for local_index in rng.sample(range(count), 5):
        examples.append(
            make_example(int(inverse_rank[local_index]), "Random positive")
        )
    return examples
